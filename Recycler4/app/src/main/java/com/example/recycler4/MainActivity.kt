            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                ...
                when(direction){
                    ...
                    ItemTouchHelper.LEFT ->{
                        Toast.makeText(applicationContext, "Item Deleted : ${newArrayList[viewHolder.adapterPosition].name}", Toast.LENGTH_SHORT).show()
                        adapter.deleteItem(viewHolder.adapterPosition)
                    }

                    ItemTouchHelper.RIGHT ->{
                        val archieveItem = newArrayList[viewHolder.adapterPosition]
                        Toast.makeText(applicationContext, "Item Archieved: ${archieveItem.name}", Toast.LENGTH_SHORT).show()
                        adapter.deleteItem(viewHolder.adapterPosition)
